 ...
 * 5 - 15 = 63 은 틀린 수식이므로 "X", 3 - 1 = 2는 옳은 수식이므로 "O" 따라서 ["O", "O", "X", "O"]를 return 합니다.
 */
public class OXQuiz {
    public static void main(String[] args) {
        String[] quiz = {"19 - 6 = 13", "5 + 66 = 71", "5 - 15 = 63", "3 - 1 = 2"};
        System.out.println(Arrays.toString(solution1(quiz)));
        System.out.println(Arrays.toString(solution2(quiz)));
    }
    private static String[] solution1(String[] quiz) {
        String[] answer = new String[quiz.length];

        for(int i = 0; i < quiz.length; i++){
            String[] str = quiz[i].split(" ");
            int num = Integer.parseInt(str[0]) + Integer.parseInt(str[2]) * (str[1].equals("+") ? 1 : -1);
            int result = Integer.parseInt(str[4]);
            answer[i] = num == result ? "O" : "X";
        }
        return answer;
    }
    private static String[] solution2(String[] quiz) {
