 ...
 *
 * XX 게임에는 피로도 시스템(0 이상의 정수로 표현합니다)이 있으며, 일정 피로도를 사용해서 던전을 탐험할 수 있습니다.
...
public class Fatigue {
    ...
    private static void dfs(int[][] dungeons, boolean[] visited, int depth, int k){
        for(int i = 0; i < dungeons.length; i++){
            int required = dungeons[i][0];
            int consume = dungeons[i][1];
            if(!visited[i] && required <= k){
                visited[i] = true;
                dfs(dungeons, visited, depth + 1, k - consume);
                visited[i] = false;
            }
        }
